...
bipartiteRL <- function(df1, df2,
  ...
  chain$files <- list(df1, df2)
  chain$comparisons <- list(list(cmpdata))

  # Store comparison and prior parameters
  chain$priors <- list(a=a, b=b, aBM=aBM, bBM=bBM)
  chain$cmpdetails <- list(fldsglobal=flds, types=types, breaks=breaks,
                           flds=list(flds1, flds2))

  attr(chain, "class") <- "bstrlstate"
